	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.CokeProcessing.coords, true) < 5 then
			if not isProcessing then
				exports['j-textui']:Help("Spauskite ~INPUT_CONTEXT~, Kad Išpakuoti Kokaina")
			end
...
function ProcessCoke()
	...
	exports.rprogress:Start("Išpakuojamas Kokainas", 10000)
	while timeLeft > 0 do
		Citizen.Wait(1000)
		timeLeft = timeLeft - 1
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				exports['j-textui']:Help("Spauskite ~INPUT_CONTEXT~, Kad Pradetumete Rinkti Kokaino Paketa")
			end
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_illegal:canPickUp', function(canPickUp)
					if canPickUp then
						...
						exports.rprogress:Start("Renkate Kokaino Paketa", 5500)
...
function SpawnCocaPlants()
	while spawnedCocaLeaf < 15 do
		...
		ESX.Game.SpawnLocalObject('prop_apple_box_01', weedCoords, function(obj)
