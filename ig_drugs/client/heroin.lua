	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.HeroinProcessing.coords, true) < 5 then
			if not isProcessing then
				exports['j-textui']:Help("Spauskite ~INPUT_CONTEXT~, kad perdirbtumete aguonas")
			end
...
function ProcessHeroin()
	...
	exports.rprogress:Start("Gaminamas heroinas", 10000)
	while timeLeft > 0 do
		Citizen.Wait(1)
		timeLeft = timeLeft - 1
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				...
				exports['j-textui']:Help("Spauskite ~INPUT_CONTEXT~, kad rinktumete aguonu derva")
			end
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_illegal:canPickUp', function(canPickUp)
					if canPickUp then
						...
						exports.rprogress:Start("Renkamos aguonos", 5500)
