	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 5 then
			if not isProcessing then
				exports['j-textui']:Help("Spauskite ~INPUT_CONTEXT~, kad dziovintumete kanapes")
			end
...
function ProcessWeed()
	...
	exports.rprogress:Start("Džiovinama žolė", 10000)
	while timeLeft > 0 do
		Citizen.Wait(1)
		timeLeft = timeLeft - 1
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				exports['j-textui']:Help("Spauskite ~INPUT_CONTEXT~, Kad Pradetumete Rinkti Žole")
			end
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_illegal:canPickUp', function(canPickUp)
					if canPickUp then
						...
						exports.rprogress:Start("Nurenkama žolė", 5500)
						...
						ESX.ShowNotification(_U('weed_processingfull'))
					end
					isPickingUp = false
